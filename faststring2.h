...
protected:
 ...
 faststring(unsigned i, char fill, unsigned len)
  {
    unsigned num_digits = (i>0) ? ((int) std::log10 ((double)i))+1 : 1; // We take care not to pass 0 to log10  
  ...
  faststring get_string_extension_or_all(char delim, unsigned fpos_minimum = 0)
  {
    unsigned fpos = this->rfind(delim);
  ...
  // The second parameter can be used to specifiy a minimum number of characters that need to
  // be found before the extension. fpos_minimum == 1: The extension cannot start at index 0.
  faststring get_string_extension(char delim, unsigned fpos_minimum = 0)
  {
    unsigned fpos = this->rfind(delim);
  ...
  // The second parameter can be used to specifiy a minimum number of characters that need to
  // be found before the extension. fpos_minimum == 1: The extension cannot start at index 0.
  faststring get_string_before_extension(char delim, unsigned fpos_minimum = 0)
  {
    unsigned fpos = this->rfind(delim);
  ...
  // The second parameter can be used to specifiy a minimum number of characters that need to
  // be found before the extension. fpos_minimum == 1: The extension cannot start at index 0.
  faststring get_string_before_extension_or_all(char delim, unsigned fpos_minimum = 0)
  {
    unsigned fpos = this->rfind(delim);
